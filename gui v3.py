...
def run_ocr():
    ...
    class VideoStream:
        ...
        def __init__(self, src="http://192.168.1.4:8080/video"):
            ...
            #         requests.get("http://192.168.1.10" + "/control?var=framesize&val=7")
            self.stopped = False
...
# Get the screen size of the user's computer
screen_width = root.winfo_screenwidth()
screen_height = root.winfo_screenheight()

# Load the photo and resize it to fit the screen size
photo = Image.open("photo3.jpeg")
photo = photo.resize((screen_width, screen_height), Image.ANTIALIAS)

# Convert the photo to a format that Tkinter can display
...
# Create a label widget to display the photo as the background
background_label = tk.Label(root, image=photo_tk)
...
button = tk.Button(root, text="Run!", font=("Arial", 24), command=run_ocr, bg="#d9d9d9")
button.pack(side=tk.LEFT, padx=10, pady=10, anchor=tk.SW)

button = tk.Button(root, text="Close!", font=("Arial", 24), command=close_every_thing, bg="#d9d9d9")
button.pack(side=tk.LEFT, padx=60, pady=10, anchor=tk.SW)
